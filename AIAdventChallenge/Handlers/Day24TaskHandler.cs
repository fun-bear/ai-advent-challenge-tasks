...
/// </summary>
public static class Day24TaskHandler
{
    ...
    [
        ...
        "Is Joey an enemy for Chandler?",
        "What is a city of living of main characters?"
    ];
        ...
        double scoreThreshold = 0.1)
    {
        ...
        response.AppendLine("=== Day24: Ответ с RAG ===");
        ...
        if (rerankedResults.Count == 0)
        {
            ...
            response.AppendLine("Я не знаю ответа на этот вопрос по имеющемуся контексту. Пожалуйста, уточните вопрос.");
    ...
    private static string BuildRagPrompt(string userQuery, List<Retriever.SearchResult> chunks)
    {
        ...
        for (var i = 0; i < chunks.Count; i++)
        {
            ...
            contextBuilder.AppendLine($"[Fragment {i + 1}] ChunkId: {c.ChunkId}, File: {c.File}, Section: {c.Section}, Score: {chunks[i].Score:F3}");
            contextBuilder.AppendLine("Fragment full text:");
...
If the context is not enough, you should request more data and don't answer question.

Provide list of fragments really used for answering in the following format (if you can give answer to question):
[Fragment 1] ChunkId: 1, File: file1.txt, Section: section1, Score: 0.999
Fragment text
---
[Fragment 2] ChunkId: 2, File: file2.txt, Section: section2, Score: 0.998
Fragment text
